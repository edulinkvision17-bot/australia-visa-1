...
import { Boxes, CircleUser, LayoutDashboard } from "lucide-react";
          ...
          {nav.subItems ? (
            ...
            nav.path && (
              <Link
                href={route(nav.path)}
                className={`relative flex items-center w-full gap-3 px-3 py-2 font-medium rounded-lg text-theme-sm group ${
                  isActive(nav.path) ? "text-brand-500 dark:text-brand-400 bg-admin-acive-nav text-admin-primary" : "text-gray-500 group-hover:text-gray-700 dark:text-gray-400 dark:group-hover:text-gray-300"
                }`}
              >
                <span
                  className={`menu-item-icon-size ${
